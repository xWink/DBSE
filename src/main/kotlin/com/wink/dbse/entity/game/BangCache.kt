...
class BangCache(
    ...
    fun add(bangEntity: BangEntity, chambers: Int) {
    ...
    private fun performBangForResult (bangEntity: BangEntity) {
        ...
        //TODO: add jams, deaths and bangs here otherwise use repository stuff
    ...
    private fun printBangResult(bangEntity: BangEntity, chambers: Int) {
        val name = jda.getUserById(bangEntity.userId!!)?.safeName() ?: "Unknown user"
        val output = getShotResultString(name, bangEntity.result!!, chambers) + getDailyResultOutputString(name, bangEntity.userId) +
    ...
    private fun compilePanicAttemptsOutput(updates: HashMap<Long, LinkedList<BangEntity>>): String {
        ...
        for (userUpdates in updates.values) {
            output += compiledIndividualPanicOutputString(userUpdates,jda.getUserById(userUpdates[0].userId
                ?: continue)
                ?: continue)
        }
        return output
